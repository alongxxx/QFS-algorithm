...
until [ $NBUE -gt $2 ]; do
	...
	TOTALNAME=$FILE"_"$COUNT"_"$FILENAME"_PF_"$NBUE"U"$CELS"C"".sim"
		...
		for bearer in $(seq 1 1 $(( ${NBUE} )))
	do 
        grep "RX "$7".*C 1"   $TOTALNAME  | grep "DST ${bearer} " |  awk '{print $8}'  > tmp
      ./compute_throughput.sh tmp $time>>tmp_2
	done 
	../../TOOLS/./make_fairness_index2  tmp_2 >> temporal
	rm tmp
	rm tmp_2
	 let COUNT=COUNT+1
	done
...
./Graph1.sh $7_$8 PF_Y1_$8_$7.dat MLWDF_Y1_$8_$7.dat EXPPF_Y1_$8_$7.dat $7-$8-Fairness-Index2 Users Fairness_Index MY_algo_Y1_$8_$7.dat MY_algo2_Y1_$8_$7.dat
...
echo  Cell-Center-FAIRNESS2 $7 REPORT FINISHED!! 
