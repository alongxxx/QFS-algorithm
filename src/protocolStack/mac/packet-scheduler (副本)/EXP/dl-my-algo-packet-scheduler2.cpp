...
double
DL_my_algo_PacketScheduler2::ComputeSchedulingMetric1 (RadioBearer *bearer, double spectralEfficiency, int subChannel,double demand , double total)
{
  //double metric;
  //metric = spectralEfficiency * 180000;
    double metric = (spectralEfficiency * 180000.)/
...
double
DL_my_algo_PacketScheduler2::EXPComputeSchedulingMetric (RadioBearer *bearer, double spectralEfficiency, int subChannel)
{
  /*
...
DL_my_algo_PacketScheduler2::RBsAllocation ()
{
  ...
  for (int s = 0; s < nbOfRBs; s++)
  {
      for (int j = 0; j < flows->size (); j++)
      {
	        my_metrics[s][j] = EXPComputeSchedulingMetric (flows->at (j)->GetBearer (),
...
void
DL_my_algo_PacketScheduler2::DoSchedule ()
{
#ifdef SCHEDULER_DEBUG
...
void
DL_my_algo_PacketScheduler2::ComputeAW ()
{
  FlowsToSchedule *flowsToSchedule = GetFlowsToSchedule ();
...
double
DL_my_algo_PacketScheduler2::GetAW (void) const
{
  return m_aW;
}
