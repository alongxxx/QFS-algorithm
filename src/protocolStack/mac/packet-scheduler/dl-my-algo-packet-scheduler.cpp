...
double
DL_my_algo_PacketScheduler::ComputeSchedulingMetric (RadioBearer *bearer, double spectralEfficiency, int demand)
{
	  ...
	  for (int j = 0; j < flows->size (); j++)
	    {
		  metrics[i][j] = ComputeSchedulingMetric (flows->at (j)->GetBearer (),
				                                   flows->at (j)->GetCqiFeedbacks().at (i),//i
	    		                                   flows->at (j)->GetDataToTransmit());
	    }
    }
