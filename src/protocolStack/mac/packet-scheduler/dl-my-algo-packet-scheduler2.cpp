...
double
DL_my_algo_PacketScheduler2::mlwdf_ComputeSchedulingMetric (RadioBearer *bearer, double spectralEfficiency,int subChannel,double credit_rate,double avg_credit ,int demand,double cqi_rate){
  ...
  if(transbit <= 328 && transbit > 144){
        if(demand > 328){
           demand_ratio=transbit/328;
        }
        if(demand <= 328 && demand > 144){     
           if(transbit > demand){
                demand_ratio=demand/transbit;
           }else{
                demand_ratio=1;     
  ...
  */
  double metric;
        ...
        if ((bearer->GetApplication ()->GetApplicationType () == Application::APPLICATION_TYPE_INFINITE_BUFFER)
          ...
          {
	          metric = (spectralEfficiency * 180000.)/1000;
          }
         else
           {
          ...
          if(bearer->GetApplication ()->GetApplicationType () == Application::APPLICATION_TYPE_TRACE_BASED){
                        metric =  (a * (HOL+1/a))
	        		 *
	        		 (spectralEfficiency * 180000.)/1000;
          }
        ...
        //int UserID = bearer->GetApplication ()->GetApplicationID ();
  //cout << "RB= " << subChannel << " "<<bearer->GetApplication ()->GetApplicationType ()<<"  UserID= " << UserID/3 << " "<< UserID <<"\t,demand = " << demand << "\t,transbit = " << transbit << "\t,demand_ratio = " << std::left << setw(12) <<demand_ratio << "\t,metric = " << std::left << setw(12) << metric << "\ttotal metric = " << std::left << setw(12) << metric*demand_ratio << ",delay = " << std::left << setw(12) << bearer->GetHeadOfLinePacketDelay () << ",credit = "<< 2-credit_rate << endl;
    return metric*demand_ratio*cqi_rate;
    //return demand_ratio;
}
