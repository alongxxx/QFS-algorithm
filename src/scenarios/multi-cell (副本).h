...
static void MultiCell (int nbCell, double radius,
                       ...
                       int seed,double data_rate)
{

  total_flow *tt_flows = total_flow::Init();
  tt_flows->set_all_flows(nbVoIP, nbVideo, nbBE, nbCBR,data_rate);

  // define simulation times
  double duration = 122;
  double flow_duration = 122;
      ...
      for (int i = 0; i < nbUE; i++)
        {
		  ...
		  std::cout << "Created UE - id " << idUE << " position " << posX << " " << posY
				  ...
				  << ", target enb " << ue->GetTargetNode ()->GetIDNetworkNode () << ", speedDirection "<< speedDirection <<std::endl;
		  ...
		  for (int j = 0; j < nbVoIP; j++)
			{
			  ...
			  else if (downlink_scheduler_type == ENodeB::DLScheduler_My_Algo2)
				{
				  QoSForMY_algo2 *qos = new QoSForMY_algo2 ();
				  qos->SetMaxDelay (maxDelay);
				  VoIPApplication[voipApplication].SetQoSParameters (qos);
				}
			  else
