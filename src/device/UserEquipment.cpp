...
bool
UserEquipment::IsInCellCenter (void)
{
  double radius=NetworkManager::Init()->GetCellByID (0)->GetRadius() * 1000.0;  
  double x = GetMobilityModel ()->GetAbsolutePosition()->GetCoordinateX() - GetCell () ->GetCellCenterPosition ()->GetCoordinateX ();
  double y = GetMobilityModel ()->GetAbsolutePosition()->GetCoordinateY() - GetCell () ->GetCellCenterPosition ()->GetCoordinateY ();
  
  double ue_radius=sqrt(x*x+y*y);
  
  return (ue_radius < (2.0/3.0)*radius )?true:false;
}
