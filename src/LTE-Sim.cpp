	  ...
	  if (strcmp(argv[1], "MultiCell")==0)
	    {
		  ...
		  int video_bit_rate = atoi(argv[13]);
	      int seed;
	      if (argc==15+1) seed = atoi(argv[14]);
	      else seed = -1;
              double data_rate=atof(argv[15]);
	      MultiCell (nbCells, radius, nbUE, nbVoIP, nbVideo, nbBE, nbCBR, sched_type, frame_struct, speed, maxDelay, video_bit_rate, seed,data_rate);
	    }
